...
pattern = re.compile(
    r"EXPORT int my_mprotect\(x64emu_t\* emu, void \*addr, "
    r"unsigned long len, int prot\)\s*\{.*?\n\}\n\n"
    r"(?=typedef struct mallinfo)",
    re.S,
)
...
replacement = r'''EXPORT int my_mprotect(x64emu_t* emu, void *addr, unsigned long len, int prot)
{
    ...
    /*
     * Android execmod compatibility fallback.
     * Dirty MAP_PRIVATE file pages can be denied when changed to executable.
     * Replace the range with anonymous memory at the same address, restore
     * its bytes, and then apply the requested execute protection.
     */
...
echo
echo "=== PATCH MARKER ==="
grep -n "BOX64_EXECMOD_FALLBACK" src/wrapped/wrappedlibc.c | head
