...
GENERATED="${HOME}/.MOBOX_TR_PHASE2B_WINE1010.generated.sh"
META="${DOWNLOADS}/MOBOX_TR_PHASE2B_WINE1010_META.txt"
KEY="${DOWNLOADS}/MOBOX_TR_PHASE2B_WINE1010_SANITIZED.txt"
FULL="${DOWNLOADS}/MOBOX_TR_PHASE2B_WINE1010_FULL_SANITIZED.txt.gz"
...
# Build a separate prefix from the known-good base. The Wine 9.3 prefix is never
# modified. Wine 10.10 may upgrade only this test copy on first launch.
...
META=/mnt/downloads/MOBOX_TR_PHASE2B_WINE1010_META.txt
...
if [ ! -f "$TEST/.phase2b_ready" ] || [ ! -f "$TEST/user.reg" ] || [ ! -d "$TEST/drive_c" ]; then
  clone_state=rebuilt
  tmp="${TEST}.tmp.$$"
  rm -rf "$tmp" "$TEST"
  mkdir -p "$tmp"
...
wine_version="$(timeout 30s "$BOX" "$WINE10_ROOT/bin/wine" --version 2>&1 | tail -1 || true)"
{
  echo "MOBOX TALESRUNNER PHASE 2B META"
  echo "================================"
  ...
  echo "WINE_VERSION_PROBE=$wine_version"
  echo "WINDOWS_VERSION_LINE=$version_line"
  echo "WINEESYNC=1"
  echo "BOX64_BINARY=/root/box64/build/box64"
  echo "BOX64_DYNACACHE_FOLDER=/root/.cache/box64-tr-v4-wine1010"
  echo "WINLATOR_BOX64RC_DISABLED=1"
...
src = src.replace('MOBOX_TR_STAGE_TRACE_V2', 'MOBOX_TR_PHASE2B_WINE1010')
src = src.replace('MOBOX TALESRUNNER STAGE TRACE V2',
                  'MOBOX TALESRUNNER PHASE 2B WINLATOR WINE 10.10 TRACE')
...
src = src.replace('BOX64_DYNACACHE_FOLDER=/root/.cache/box64-tr-v4-862fef5',
                  'BOX64_DYNACACHE_FOLDER=/root/.cache/box64-tr-v4-wine1010')
...
required = [
    ...
    'export WINEESYNC=1',
    'BOX64_DYNACACHE_FOLDER=/root/.cache/box64-tr-v4-wine1010',
    'MOBOX_TR_PHASE2B_WINE1010_SANITIZED.txt',
    'MOBOX_TR_PHASE2B_WINE1010_FULL_SANITIZED.txt.gz',
]
...
echo
echo "PHASE2B_TRACE_RC=$trace_rc"
echo "META=$META"
echo "KEY_LOG=$KEY"
echo "FULL_LOG=$FULL"
exit "$trace_rc"
