...
STATUS="${DOWNLOADS}/MOBOX_TR_WINE1010_CUSTOM_PROOT_SYSVIPC_V4_STATUS.txt"
PROBE="${DOWNLOADS}/MOBOX_TR_CUSTOM_PROOT_SYSVIPC_V4_PROBE.txt"
...
[ -x "$CUSTOM_PROOT" ] || {
  ...
  echo "Run BUILD_PROOT_SYSVIPC_SELFEXE_FIX.sh first" >&2
  exit 5
}
...
# Verify the patched helper before starting the authenticated game path.
set +e
PD_PROOT_BIN="$CUSTOM_PROOT" proot-distro login debian \
...
if shmid < 0:
    ...
    print(f"CUSTOM_PROOT_SYSVIPC_V4_FAIL shmget errno={err} {os.strerror(err)}")
    sys.exit(41)
print(f"CUSTOM_PROOT_SYSVIPC_V4_SHMID={shmid}")
if libc.shmctl(shmid, IPC_RMID, None) != 0:
    err = ctypes.get_errno()
    print(f"CUSTOM_PROOT_SYSVIPC_V4_FAIL shmctl errno={err} {os.strerror(err)}")
    sys.exit(42)
print("CUSTOM_PROOT_SYSVIPC_V4_PROBE_OK")
PY
PROBE_DEBIAN
probe_rc=$?
set -e
termux-media-scan "$PROBE" >/dev/null 2>&1 || true
if [ "$probe_rc" -ne 0 ] || ! grep -q '^CUSTOM_PROOT_SYSVIPC_V4_PROBE_OK$' "$PROBE"; then
  echo "CUSTOM_PROOT_SYSVIPC_V4_PROBE_FAILED"
  echo "REPORT=$PROBE"
  tail -n 80 "$PROBE" || true
  exit 7
fi

echo "CUSTOM_PROOT_SYSVIPC_V4_PROBE_OK"
...
STATUS=/mnt/downloads/MOBOX_TR_WINE1010_CUSTOM_PROOT_SYSVIPC_V4_STATUS.txt
...
export BOX64_DYNACACHE_FOLDER=/root/.cache/box64-tr-v4-wine1010-custom-proot-sysvipc-v4
...
{
  echo "MOBOX TALESRUNNER WINE 10.10 CUSTOM PROOT SYSVIPC V4"
  echo "====================================================="
  date -Iseconds
...
echo "CUSTOM_PROOT_SYSVIPC_V4_WRAPPER_EXIT=$wrapper_status"
echo "STATUS=$STATUS"
echo "PROBE=$PROBE"
exit "$wrapper_status"
