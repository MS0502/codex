...
def copy_group(rootfs: Path, prefix: Path, data: dict[str, list[str]], key: str, source: str) -> int:
    names = data.get(key)
    ...
    for name in names:
        ...
        dst = dst_dir / name
        if not src.is_file():
            raise RuntimeError(f"common DLL source missing: {src}")
...
def main() -> int:
    ...
    system32 = copy_group(rootfs, prefix, data, "system32", "x86_64-windows")
    syswow64 = copy_group(rootfs, prefix, data, "syswow64", "i386-windows")
