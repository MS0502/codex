...
BUILD_SRC="$HOME_DIR/.cache/proot-sysvipc-fix-v2-build/proot-5.1.107.84/src"
BUILT_LOADER="$BUILD_SRC/loader/loader"
BUILT_LOADER32="$BUILD_SRC/loader/loader-m32"
CUSTOM_LOADER="$CUSTOM_DIR/loader"
CUSTOM_LOADER32="$CUSTOM_DIR/loader32"
PROOT_TMP="$HOME_DIR/.cache/proot-sysvipc-v4-tmp"
REPORT="$DOWNLOADS/PROOT_SYSVIPC_MATCHED_LOADER_V4_REPORT.txt"

mkdir -p "$DOWNLOADS" "$CUSTOM_DIR" "$PROOT_TMP"
chmod 700 "$PROOT_TMP"
: >"$REPORT"

for required in "$CUSTOM_PROOT" "$BUILT_LOADER"; do
  [ -f "$required" ] || {
    echo "ERROR: missing required file: $required" | tee -a "$REPORT" >&2
    exit 2
  }
done

install -m 700 "$BUILT_LOADER" "$CUSTOM_LOADER"
if [ -f "$BUILT_LOADER32" ]; then
  install -m 700 "$BUILT_LOADER32" "$CUSTOM_LOADER32"
fi
...
{
  echo "PROOT SYSVIPC MATCHED LOADER V4 PROBE"
  echo "======================================"
  date -Iseconds
  ...
  file "$CUSTOM_PROOT"
  file "$CUSTOM_LOADER"
  [ ! -f "$CUSTOM_LOADER32" ] || file "$CUSTOM_LOADER32"
  echo
} >>"$REPORT" 2>&1
...
env \
  PATH="$CUSTOM_PATH" \
  PROOT_LOADER="$CUSTOM_LOADER" \
  PROOT_LOADER_32="$CUSTOM_LOADER32" \
...
if shmid < 0:
    ...
    print(f"MATCHED_LOADER_V4_SYSVIPC_FAIL shmget errno={err} {os.strerror(err)}")
    sys.exit(31)

print(f"MATCHED_LOADER_V4_SYSVIPC_SHMID={shmid}")
rc = libc.shmctl(shmid, IPC_RMID, None)
if rc != 0:
    err = ctypes.get_errno()
    print(f"MATCHED_LOADER_V4_SYSVIPC_FAIL shmctl errno={err} {os.strerror(err)}")
    sys.exit(32)

print("MATCHED_LOADER_V4_SYSVIPC_PROBE_OK")
PY
PROBE_DEBIAN
probe_rc=$?
set -e
...
if [ "$probe_rc" -ne 0 ] || ! grep -q '^MATCHED_LOADER_V4_SYSVIPC_PROBE_OK$' "$REPORT"; then
  echo "MATCHED_LOADER_V4_SYSVIPC_PROBE_FAILED"
  echo "REPORT=$REPORT"
  tail -n 120 "$REPORT" || true
  exit 4
fi

echo "MATCHED_LOADER_V4_SYSVIPC_PROBE_OK"
echo "REPORT=$REPORT"
